...
- **LLM polish.** Optional `src/llm.js` sends the menu to a bring-your-own-key LLM provider (Anthropic, Google AI Studio, OpenAI, Kimi, or DeepSeek) for atmospheric descriptions. Providers other than Anthropic share one OpenAI-compatible request path; adding another is a new entry in the `PROVIDERS` registry. Nothing is stored server-side.
