...
6. The full world state and seed round-trip through the URL, so any menu can be shared as a link (Share link button), copied as plain text for session notes (Copy text), or printed / saved as PDF (Print).
...
The repo ships with two visible packs, both off by default. **Fabled fare** adds monster meats and fey provisions (hippogriff steak, slime soup, kraken fritters, griffin eggs) for tables that like their inns a little enchanted; every creature is folklore, not someone's rulebook. **Mog** is the author's campaign setting; check it to add Mog's regional cuisine. Forks can drop their own packs into `data/flavor_packs/` and add an entry to `index.json`: no code changes required. See `docs/DESIGN.md` for the pack schema.
...
│       ├── fabled.json         # monster meats and fey provisions (off by default)
